#!/usr/bin/env bash
#
# Automates the extraction of Nintendo Switch NSP/NSZ/XCI/XCZ file content
# into directories.
#
...
AdditionalParameters=(--alwaysParseCnmt --titlekeys --quick-verify)
...
echo
echo " o Destination directory path where to extract the content of NSP/NSZ/XCI/XCZ files:"
...
if [[ "${EnableRecursiveSearch,,}" == "true" ]]; then
	mapfile -d '' -t Files < <(find "$SrcDirectoryPath" -type f \( -iname "*.nsp" -o -iname "*.nsz" -o -iname "*.xci" -o -iname "*.xcz" \) -print0)
else
	mapfile -d '' -t Files < <(find "$SrcDirectoryPath" -maxdepth 1 -type f \( -iname "*.nsp" -o -iname "*.nsz" -o -iname "*.xci" -o -iname "*.xcz" \) -print0)
fi

for File in "${Files[@]}"; do
	echo "Extracting \"$File\"..."
	echo
	"$NszPath" --extract "$File" --output "$DstDirectoryPath" "${AdditionalParameters[@]}" \
		|| print_error_and_exit "nsz failed to extract file: \"$File\""
done
