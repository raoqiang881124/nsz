...
class BktrRelocationEntry:
    ...
    def printInfo(self, maxDepth=3, indent=0):
        tabs = "\t" * indent
        Print.info(
            "%sRelocation Entry %s %x = %x"
            % (
                tabs,
                "Patch" if self.isPatch else "Base",
                self.physicalOffset,
                self.virtualOffset,
            )
        )
...
class BktrSubsectionEntry:
    ...
    def printInfo(self, maxDepth=3, indent=0):
        tabs = "\t" * indent
        Print.info(
            "%sSubsection Entry %d, CTR = %x" % (tabs, self.virtualOffset, self.ctr)
        )
...
class Bktr(Header):
    def __init__(
        ...
        super(Bktr, self).__init__(
            path, mode, cryptoType, cryptoKey, cryptoCounter, nca
        )
