...
class Pfs0Stream(BaseFile):
    ...
    def add(self, name, size, pleaseNoPrint=None):
        if self.written:
            ...
            self.written = False
        Print.info(
            f"[ADDING]	 {name} {hex(size)} bytes to PFS0 at {hex(self.addpos)}",
            pleaseNoPrint,
        )
        partition = self.partition(self.addpos, size, n=BaseFile())
        self.files.append(
            {"name": name, "size": size, "offset": self.addpos, "partition": partition}
        )
        self.addpos += size
        return partition
    ...
    def getHeader(self):
        ...
        stringTable = stringTableNonPadded + (
            "\x00" * (stringTableSizePadded - len(stringTableNonPadded))
        )
...
class Pfs0(BaseFs):
    def __init__(
        ...
        super(Pfs0, self).__init__(
            buffer, path, mode, cryptoType, cryptoKey, cryptoCounter
        )

        if buffer:
            self.size = int.from_bytes(
                buffer[0x48:0x50], byteorder="little", signed=False
            )
            self.sectionStart = int.from_bytes(
                buffer[0x40:0x48], byteorder="little", signed=False
            )
    ...
    def open(
        ...
        for i in range(fileCount):
            ...
            name = (
                stringTable[nameOffset:stringEndOffset]
                .decode("utf-8")
                .rstrip(" \t\r\n\0")
            )
            ...
            self.files.append(
                self.partition(offset + self._headerSize, f.size, f, autoOpen=False)
            )
