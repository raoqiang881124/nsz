...
class SectionTableEntry:
    def __init__(self, d):
        ...
        self.mediaEndOffset = int.from_bytes(
            d[0x4:0x8], byteorder="little", signed=False
        )
...
class NcaHeader(File):
    def __init__(
        self, path=None, mode=None, cryptoType=-1, cryptoKey=-1, cryptoCounter=-1
    ):
        ...
        super(NcaHeader, self).__init__(
            path, mode, cryptoType, cryptoKey, cryptoCounter
        )
    ...
    def open(
        ...
        super(NcaHeader, self).open(
            file, mode, cryptoType, cryptoKey, cryptoCounter, meta_only
        )
        self.rewind()
        ...
        self.masterKey = (
            self.cryptoType if self.cryptoType > self.cryptoType2 else self.cryptoType2
        ) - 1
        ...
        if self.hasTitleRights():
            ...
            if (
                titleRightsTitleId in Titles.keys()
                and Titles.get(titleRightsTitleId).key
            ):
...
class Nca(File):
    def __init__(
        self, path=None, mode="rb", cryptoType=-1, cryptoKey=-1, cryptoCounter=-1
    ):
        self.header = None
    ...
    def open(
        ...
        self.partition(
            0x0, 0xC00, self.header, Fs.Type.Crypto.XTS, uhx(Keys.get("header_key"))
        )
