...
class BaseFs(File):
    ...
    def __getitem__(self, key):
        if isinstance(key, str):
            for f in self.files:
                if (hasattr(f, "name") and f.name == key) or (
                    hasattr(f, "_path") and f._path == key
                ):
                    return f
    ...
    def getEncryptionSections(self):
        ...
        if self.hasBktr():
            ...
            for entry in self.bktrSubsection.getAllEntries():
                ...
                sections.append(
                    EncryptedSection(
                        self.realOffset() + entry.virtualOffset,
                        entry.size,
                        self.cryptoType,
                        self.cryptoKey,
                        ctr,
            ...
            if len(sections) == 0:
                ...
                sections.append(
                    EncryptedSection(
                        offset,
                        (sectionOffset + self.size) - offset,
                        self.cryptoType,
                        self.cryptoKey,
                        self.cryptoCounter,
                    )
    ...
    def hasBktr(self):
        return (
            False if self.bktrSubsection is None else True
        ) and self.bktrSubsection.isValid()
    ...
    def open(
        ...
        if self.bktr1Buffer:
            try:
                self.bktrRelocation = Bktr.Bktr1(
        ...
        if self.bktr2Buffer:
            try:
                self.bktrSubsection = Bktr.Bktr2(
