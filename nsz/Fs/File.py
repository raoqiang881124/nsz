...
class BaseFile:
    ...
    def setupCrypto(self, cryptoType=-1, cryptoKey=-1, cryptoCounter=-1):
        ...
        if (
            ...
            or self.cryptoType == nsz.Fs.Type.Crypto.BKTR
        ):
            if self.cryptoKey:
                self.crypto = aes128.AESCTR(
                    self.cryptoKey, nonce=self.cryptoCounter.copy()
                )
                self.cryptoType = nsz.Fs.Type.Crypto.CTR
...
class BufferedFile(BaseFile):
    def __init__(
        ...
        super(BufferedFile, self).__init__(
    ...
    def read(self, size=None, direct=False):
        ...
        if (
            ...
            self.flushBuffer()
            self._bufferOffset = (
                self._relativePos // self._bufferAlign
            ) * self._bufferAlign
...
class MemoryFile(File):
    def __init__(
        self, buffer, cryptoType=-1, cryptoKey=-1, cryptoCounter=-1, offset=None
    ):
        ...
        self.setupCrypto(
            cryptoType=cryptoType, cryptoKey=cryptoKey, cryptoCounter=cryptoCounter
        )
...
class CryptoFile(BufferedFile):
    def __init__(
        ...
        super(CryptoFile, self).__init__(
            path, mode, cryptoType, cryptoKey, cryptoCounter
        )
...
class AesXtsFile(CryptoFile):
    def __init__(
        ...
        super(AesXtsFile, self).__init__(
            path, mode, cryptoType, cryptoKey, cryptoCounter
        )
...
class AesCtrFile(CryptoFile):
    def __init__(
        ...
        super(AesCtrFile, self).__init__(
            path, mode, cryptoType, cryptoKey, cryptoCounter
        )
